...
syspath = '/sys/bus/i2c/drivers/ads1015/1-0048/'

FileName = []

for i in range(num_chan):
   # TempName  = glob.glob (syspath + 'in_voltage' + args.adc[i] + '_raw')
    TempName = glob.glob (syspath + 'in' + args.adc[i] + '_input')
...
#    h.newpin("temp" + str(i), hal.HAL_FLOAT, hal.HAL_OUT)
     h.newpin("raw", hal.HAL_FLOAT, hal.HAL_OUT)
...
while 1:
    try:
        for i in range(num_chan):
           ...
           # h['raw' + str(i)] = ADC_IN
            h['raw'] = ADC_IN
            #ADC_V = float(ADC_IN) * 1.8 / 4096.0 # very weird stuff
	    ADC_V = float(ADC_IN) / 1000 # ADC direct output in mV, so need to convert to V 
